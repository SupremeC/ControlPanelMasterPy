...
import logging
import requests
from weather import Forecast, RawForecast
...
# {lightning likely} {wind}
    ...
    def _forecast_as_str(f: Forecast) -> str:
        winterTime = True if datetime.now().month > 9 or datetime.now().month < 4 else False
        tempstr = f"lows about {int(round(f.low_c))}" if winterTime else f"highs about {int(round(f.high_c))}, "
    ...
    def _parse_weather_json(raw: dict) -> Forecast:
        ...
        if len(d) > 6: f = SMHI._filter_by_time(d, 7, 21)
    ...
    def _get_downfall_descr(timeSeries) -> str:
        ...
        rrain = [
            obj for obj in timeSeries
            if any(param["name"] == "pcat" and 2 < param["values"][0] <= 6 for param in obj["parameters"])
        ]

        rsnow = [
            obj for obj in timeSeries
            if any(param["name"] == "pcat" and 1 <= param["values"][0] < 3 for param in obj["parameters"])
        ]
        if len(rrain) > 0:
        ...
        if rain_avg > 0.1:
            if rain_avg < 4: msg += "some rain"
            elif rain_avg >=4 and rain_avg <10: msg += "rain"
            else: msg += "heavy rain"
            if len(rrain) < 4: msg += " part of the day."
            elif rain_avg > .2: msg += " most of the day."


        if snow_avg > .1:
            if snow_avg < 3: msg += "some snow"
            elif snow_avg >=3 and snow_avg <10: msg += "snowy"
            else: msg += "heavy snowing"
            if len(rsnow) < 4: " part of the day."
            elif snow_avg > .3: msg += " most of the day."
        return msg
    ...
    def _get_cloudiness(timeSeries) -> str:
        ...
        if c_notcloudy > 0 and (c_medcloudy + c_verycloudy)<.3:
            return "sunny"
        if abs(c_notcloudy - (c_medcloudy + c_verycloudy)) < .2:
            return "half clear"
        else:
            return "cloudy"
    ...
    def _filter_by_time(timeSeries, start_h: int, end_h: int):
        ...
        for ts in timeSeries:
            if (ts["validTime"].time() >= time(hour=start_h)
                and ts["validTime"].time() <= time(hour=end_h)):
                items.append(ts)
        return items
    ...
    def __get_weather_forecast() -> str:
        ...
        endpoint = "api/category/pmp3g/version/2/geotype/point/lon/{long}/lat/{lat}/data.json"
